...
if(isset($_GET['id']))
{
	...
	if(stripos($topicd['nom'], "[NOPE]")	!== FALSE) { 
		$ADMINBYPASS = true; 
		$topicd['nom'] = str_replace('[NOPE]', '', $topicd['nom']); 
	}
	if(!empty($topicd['id']))
	{
		...
		<div class="neo-center">
			<div style="width:auto;" class="neo-xbackground neo-xtransforme-2  ">
				<div class="neo-xbackground neo-xtransforme-1 " style="padding:1px;">
					<p class="neo-text-header-small  neo-xtransforme-2" style="margin-top:10px;text-transform: uppercase;"><b>Post: <?=$topicd['nom'];?></b></p>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="neo-center neo-xbackground neo-radius" style="padding:15px;">
	<?php if(!isset($ADMINBYPASS)) { ?><h3>Sujet : <?php echo $topicd['nom']; ?></h3><br/><?php } ?>
	<div class="row">
		<div class="col-md-2">
	...
	<div class="row">
		...
		<div class="col-md-2">
			<form action="?&action=signalement_topic" method="post">
				...
				<button type="submit" class="neo-button neo-red">Signaler !</button>
			</form>
		</div>
		<?php }?>
	<?php if(isset($_Joueur_) && ($_Joueur_['pseudo'] == $topicd['pseudo'] OR ($_Joueur_['rang'] == 1 OR $_PGrades_['PermsForum']['general']['editTopic'] == true) AND !$_SESSION['mode']))
	{
		?><div class="col-md-2"><form action="?action=editForum" method="post">
			...
			<button type="submit" class="neo-button neo-green">Editer le topic</button>
		</form>
		</div>
		<?php 
	}
		...
		<form action="?action=remove_topic" method="post">
			...
			<a class="neo-button neo-red" role="button" data-toggle="collapse" href="#topic_<?php echo $id; ?>" aria-expanded="false" aria-controls="collapseExample" >
				Supprimer ce topic ? </a>
				<div class="collapse" id="topic_<?php echo $id; ?>">
					<div class="well">
						<button type="submit" class="neo-button neo-red">Confirmer la suppression du topic </button>
					</div>
				</div>
		</form>
		</div><?php
		...
		if(array_search($_Joueur_['pseudo'], array_column($countlike, 'pseudo')) === FALSE AND array_search($_Joueur_['pseudo'], array_column($countdislike, 'pseudo')) === FALSE AND $_Joueur_['pseudo'] != $topicd['pseudo'])
		{
			...
			<div class="row justify-content-end"><div class="col-md-1">
					<form class="form-inline" action="?&action=like" method="post">
						...
						<button type="submit" class="neo-button neo-blue" title="J'aime" ><i class="far fa-thumbs-up"></i></button>
					</form></div><div class="col-md-1">
					<form class="form-inline" action="?&action=like" method="post">
						<input type="hidden" name="choix" value="2" />
						<input type="hidden" name="type" value="1" />
						<input type="hidden" name="id_answer" value="<?php echo $topicd['id']; ?>" />
						<button type="submit" class="neo-button neo-blue" title="Je n'aime pas"><i class="far fa-thumbs-down"></i></button>
					</form>
				</div>
			</div>
			<?php
			...
			?><div class="row justify-content-end">
				<div class="col-md-1">
					<form class='form-inline' action="?&action=unlike" method="post">
						<input type="hidden" name="id_answer" value="<?php echo $topicd['id']; ?>" />
						<input type="hidden" name="type" value="1" />
						<button type="submit" class="neo-button neo-blue" title="Ne plus aimer">Retirer</button>
					</form>
				</div>
			</div><?php
	...
	if(count($answerd) != 0) { echo '<div class="neo-center neo-xbackground neo-radius" style="padding:15px;">'; }
	...
	{ ?>
		...
		<div class="col-md-2">
			...
			elseif($_Joueur_['pseudo'] != $answerd[$i]['pseudo'] AND $test_votedata['count'] > 0)
			{
				...
				<form class='form-inline' action="?&action=unlike" method="post">
					<input type="hidden" name="id_answer" value="<?php echo $answerd[$i]['id']; ?>" />
					<button type="submit" class="neo-button neo-green" title="Ne plus aimer">Retirer</button>
				</form></div>
				<?php
			}
			if($_Joueur_['pseudo'] === $answerd[$i]['pseudo'] OR ($_PGrades_['PermsForum']['moderation']['editMessage'] == true OR $_Joueur_['rang'] == 1) AND !$_SESSION['mode'])
			{
				?><div class="col-md-2"><form action="?action=editForum" method="post">
					...
					<button type="submit" class="neo-button neo-green">Editer ce message</button>
				</form></div>
				<?php 
			}
			...
			<div class="col-md-2">
				<form action="?action=remove_answer" method="post">
					...
					<a role="button" class="neo-button neo-red" data-toggle="collapse" href="#answer_<?php echo $answerd[$i]['id']; ?>" aria-expanded="false" aria-controls="collapseExample">
						Supprimer ce message ? </a>
						<div class="collapse" id="answer_<?php echo $answerd[$i]['id']; ?>">
							<div class="well">
								<button type="submit" class="neo-button neo-red">Confirmer </button>
							</div>
						</div>
				</form></div><?php
			}
	...
	<?php  if(count($answerd) != 0) { echo '</div>'; }
	...
	<form action="?&action=post_answer" method="post">
		...
		<div class="form-group row">
			<div class="col-md-12 text-center">
				<button type="submit" class="neo-button neo-green">Poster votre réponse</button>
			</div>
		</div>
		</p>
	</form>
