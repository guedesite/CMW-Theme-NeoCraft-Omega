<?php if(!isset($_GET['id'])) header('Location: ?page=erreur&erreur=16');
	...
	if(!(($_Joueur_['rang'] == 1 AND !$_SESSION['mode']) OR $_PGrades_['PermsDefault']['forum']['perms'] >= $categoried['perms'] OR $categoried['perms'] == 0)) header('Location: ?page=erreur&erreur=7');?>sDefault']['forum']['perms'] >= $categoried['perms'] OR $categoried['perms'] == 0)) header('Location: ?page=erreur&erreur=7');?>
			...
			<div class="neo-center">
				<div style="width:auto;" class="neo-xbackground neo-xtransforme-2  ">
					<div class="neo-xbackground neo-xtransforme-1 " style="padding:1px;">
						<p class="neo-text-header-small  neo-xtransforme-2" style="margin-top:10px;text-transform: uppercase;"><b>Forum: <?=$categoried['nom'];?></b></p>
					</div>
				</div>
			</div>
		</div>
	<div class="neo-background-cale" >
		...
		if(($_Joueur_['rang'] == 1 OR $_PGrades_['PermsForum']['general']['addSousForum'] == true) AND !$_SESSION['mode'] && !isset($id_sous_forum))
		{
			...
			<div class="col-md-offset-8 col-md-4">
				<a class="neo-button neo-green" role="button" data-toggle="collapse" href="#sous_cat" aria-expanded="false" aria-controls="collapseExample">
				  Créez un sous-forum
				</a>
			</div>
						...
						<form action="?action=create_sf" method="post">
							...
							<div class="row">
								<div class="col-md-offset-4 col-md-6">
									<button type="submit" class="neo-button neo-green">Créer un sous-forum</button>
								</div>
							</div>
						</form>
					</div>
				...
				for($i = 0; $i < count($topicd); $i++)
				{
					...
					<tr>
						...
						<td><a href="?&page=post&id=<?php echo $topicd[$i]['id']; ?>"><?php if(isset($topicd[$i]['prefix']) && $topicd[$i]['prefix'] != 0)
						{
							...
							echo ' '.str_replace('[NOPE]', ' ', $topicd[$i]['nom']); ?></a><br/><p style="margin-bottom: 0px;"><small><a href="?page=profil&profil=<?=$topicd[$i]['pseudo'];?>"><?=$topicd[$i]['pseudo'];?></a>, le <?=$_Forum_->getDateConvert($topicd[$i]['date_creation']);?></small></p></td>
		...
		<form action="?&action=create_topic" method="post">
			...
			<div class="form-group row text-center">
				<div class="col-sm-offset-2 col-sm-10">
					<button type="submit" class="neo-button neo-green">Poster</button>
				</div>
			</div>
		</form>
		<?php 
